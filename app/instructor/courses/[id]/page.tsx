...
export default function CourseDetailsPage() {
  ...
  interface Review {
    ...
    comment: string;
    created_at: string;
    users?: { name: string } | { name: string }[]; // Can be single object or array
  }
  ...
  // Move fetchReviews outside of fetchCourseDetails so it can be called from submitReview
            ...
            {reviews.map((review) => (
              <li key={review.id} className="mb-4 border p-4 rounded shadow">
                <p className="font-medium">
                  User: {Array.isArray(review.users) ? review.users[0]?.name || "Unknown" : review.users?.name || "Unknown"}
                </p>
